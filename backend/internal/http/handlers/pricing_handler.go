...
func (h *PricingHandler) UpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	if err := httputil.DecodeJSON(w, r, &payload); err != nil {
		if h.logger != nil {
			h.logger.Warn().Err(err).Str("tenant_id", claims.TenantID.String()).Msg("invalid pricing settings payload")
		}
		httputil.RespondError(
			w,
			http.StatusBadRequest,
			pricingInvalidBodyMessage,
			httputil.WithErrorCode("PRECIFICACAO_CONFIG_CORPO_INVALIDO"),
...
func (h *PricingHandler) Suggest(w http.ResponseWriter, r *http.Request) {
	...
	if err := httputil.DecodeJSON(w, r, &req); err != nil {
		if h.logger != nil {
			h.logger.Warn().Err(err).Str("tenant_id", claims.TenantID.String()).Msg("invalid pricing suggestion payload")
		}
		httputil.RespondError(
			w,
			http.StatusBadRequest,
			pricingInvalidBodyMessage,
			httputil.WithErrorCode("PRECIFICACAO_SIMULACAO_CORPO_INVALIDO"),
			httputil.WithFieldError("body", err.Error()),
		)
		return
	}
